...
func (s *TransactionDBTestSuite) SetupSuite() {
	...
	db.Exec("Create table transactions (id varchar(225), account_id_from varchar(225), account_id_to varchar(225), amount int, created_at date)")

	client, _ := entity.NewClient("Roger", "eu@email.com")
	s.Nil(err)
	s.client = client

	client2, _ := entity.NewClient("Roger2", "tu@email.com")
	s.Nil(err)
	s.client2 = client2
